...
export function DailyTokensSection(props: DailyTokensSectionProps) {
  ...
  const { data: dailyTokenData, isLoading } = useQuery({
    queryKey: ['dashboard', 'daily-tokens', timeRange, isAdmin, includeCache],
    queryFn: () =>
      isAdmin
        ? getDailyTokenData({ ...timeRange, include_cache: includeCache })
        : getSelfDailyTokenData({ ...timeRange, include_cache: includeCache }),
