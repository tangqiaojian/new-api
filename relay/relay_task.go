...
func tryRealtimeFetch(c *gin.Context, task *model.Task, isOpenAIVideoAPI bool) []byte {
	...
	if !snap.Equal(task.Snapshot()) {
		...
		if updateErr != nil {
			logger.LogError(c, fmt.Sprintf("tryRealtimeFetch UpdateWithStatus failed for task %s: %s", task.TaskID, updateErr.Error()))
		} else if won && isTerminal && snap.Status != task.Status {
			// 用户主动 fetch 触发的终态迁移必须与轮询路径一样完成计费收尾：
			// 失败退款、成功结算（差额/渠道池/订阅 token）。终态任务从此退出
			// 轮询队列，错过收尾费用就永远挂账。
			service.SettleTerminalTaskBilling(c, task, ti)
		}
	}
